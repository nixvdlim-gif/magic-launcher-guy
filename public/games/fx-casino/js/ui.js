...
export function updateBalancesUI() {
  const formatted = (window.__FX_CCY__||'$') + state.account.balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
export function updateStakeUI() {
  document.getElementById('amount-text-stake').innerText = (window.__FX_CCY__||'$') + state.stakeAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const payout = state.stakeAmount * 1.0;
  const infoPayout = document.getElementById('info-payout');
  if (infoPayout) {
    infoPayout.innerText = '+' + (window.__FX_CCY__||'$') + payout.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  }
}
  ...
  async function startTutoSimulation(type) {
    ...
    tutoInterval = setInterval(() => {
      if (tutoStepIndex < 7) {
        ...
        if (isWin) {
          synth.playWin();
          if (tutoFeedback) tutoFeedback.innerHTML = `<span class="styled-97">SIMULATED TAKEUOUT! +${(window.__FX_CCY__||'$')}100</span>. Great trend-aligned trade!`;
        } else {
          synth.playLoss();
...
export function setupUIInteractions(chartInstance) {
  ...
  if (steelBtn) {
    ...
    steelBtn.addEventListener('dblclick', async () => {
      synth.init();
      const _c = (window.__FX_CCY__||'$');
      const userInput = await showCustomPrompt("CUSTOM STAKE", `Enter wager amount (${_c}${CONFIG.MIN_STAKE} to ${_c}${CONFIG.MAX_STAKE}):`, state.stakeAmount.toFixed(2));
      if (userInput !== null) {
        ...
        if (!isNaN(parsed) && parsed >= CONFIG.MIN_STAKE && parsed <= CONFIG.MAX_STAKE) {
          ...
          await showCustomAlert("INVALID ENTRY", `Wager must range between ${_c}${CONFIG.MIN_STAKE} and ${_c}${CONFIG.MAX_STAKE}.`);
        }
      }
    });
  }
  ...
  document.querySelectorAll('.btn-balance-reset-trigger').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      state.tradeHistory = [];
      updateBalancesUI();
      await showCustomAlert("BALANCE TOPPED UP", `Your simulated account balance has been reset to ${(window.__FX_CCY__||'$')}${CONFIG.BASE_BALANCE.toFixed(2)}.`);
    });
  });
