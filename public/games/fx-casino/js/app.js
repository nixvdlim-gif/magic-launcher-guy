...
/* ====== Lovable wallet bridge ====== */
const bridge = {
  ready: false,
  pending: null, // { resolve }
  post(msg) { try { window.parent && window.parent.postMessage(msg, '*'); } catch (_) {} },
  requestBet(stake, direction) {
    return new Promise((resolve) => {
      this.pending = { resolve };
      this.post({ type: 'fx:placeBet', stake, direction });
      // safety timeout
      setTimeout(() => {
...
window.addEventListener('message', (e) => {
  const d = e.data || {};
  if (d.type === 'fx:setBalance' && typeof d.amount === 'number') {
    state.account.balance = d.amount;
    updateBalancesUI();
  } else if (d.type === 'fx:setConfig') {
    if (typeof d.min_bet === 'number') CONFIG.MIN_STAKE = d.min_bet;
    if (typeof d.max_bet === 'number') CONFIG.MAX_STAKE = d.max_bet;
    if (Array.isArray(d.preset_stakes) && d.preset_stakes.length) {
      CONFIG.STAKE_PRESETS = d.preset_stakes;
      CONFIG.DEFAULT_STAKE = d.preset_stakes[Math.min(2, d.preset_stakes.length - 1)];
      state.stakeAmount = CONFIG.DEFAULT_STAKE;
    }
    if (typeof d.currency === 'string') window.__FX_CCY__ = d.currency;
    updateStakeUI();
    updateBalancesUI();
  } else if (d.type === 'fx:betResult' && bridge.pending) {
    bridge.pending.resolve(d);
    bridge.pending = null;
  } else if (d.type === 'fx:betDenied' && bridge.pending) {
    bridge.pending.resolve(null);
    bridge.pending = null;
    if (d.error) showCustomAlert('BET REJECTED', d.error);
  }
});
...
async function handleOpenPosition(type) {
  ...
  if (state.stakeAmount > state.account.balance) {
    showCustomAlert('INSUFFICIENT BALANCE', 'Top up your wallet to place this trade.');
    return;
  }

  // Disable instantly to prevent double-click while server decides
  const _buy = document.getElementById('btn-action-buy');
  const _sell = document.getElementById('btn-action-sell');
  if (_buy) _buy.disabled = true;
  if (_sell) _sell.disabled = true;

  const result = await bridge.requestBet(state.stakeAmount, type);
  if (!result || !result.ok) {
    updateActionButtonsState();
    return;
  }
  ...
  if (profitWidgetVal) {
    profitWidgetVal.innerText = (window.__FX_CCY__||'$') + '0.00';
...
function startSimulatorHeartbeat() {
  setInterval(() => {
    ...
    if (state.activeTrade && state.activeTrade.status === 'OPEN') {
      ...
      const profitWidgetVal = document.getElementById('widget-profit');
      if (profitWidgetVal) {
        profitWidgetVal.innerText = (state.activeTrade.profit >= 0 ? '+' : '') + (window.__FX_CCY__||'$') + state.activeTrade.profit.toFixed(2);
    ...
    if (state.tickIndex >= CONFIG.TICKS_PER_CANDLE) {
      ...
      if (state.activeTrade && state.activeTrade.status === 'OPEN') {
        if (state.guidedCandleIndex >= state.guidedCandles.length) {
          ...
          const finalPayoutProfit = typeof state.activeTrade.serverProfit === 'number'
            ? state.activeTrade.serverProfit
            : (isWin ? state.stakeAmount : -state.stakeAmount);
          // Server already updated wallet; tell parent to refresh, do not double-credit locally.
          bridge.post({ type: 'fx:tradeSettled', won: isWin, profit: finalPayoutProfit });
