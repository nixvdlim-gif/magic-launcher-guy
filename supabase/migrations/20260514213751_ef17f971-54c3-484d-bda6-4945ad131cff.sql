...
BEGIN
  ...
  IF v_status <> 'playing'::public.room_status THEN
    RETURN;
  END IF;
...
BEGIN
  ...
  IF v_room.status <> 'playing'::public.room_status THEN
    RETURN jsonb_build_object('skipped', false, 'reason', 'not_playing');
  END IF;

  IF v_room.turn_started_at IS NULL THEN
    UPDATE public.game_rooms
    SET turn_started_at = now(), updated_at = now()
    WHERE id = _room_id;
